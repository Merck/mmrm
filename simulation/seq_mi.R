...
rr_seq_imp <- function(db_comb, db_long, M){
  ...
  # For control group, conduct sequential for obs; conduct ~ X+Y1 for imputed
  dep_var <- list()
  for(j in 1:p){
    if(j == 1){
  ...
  imp_fn <- function(){
    ...
    for(j in 1:p){
      ...
      if(j > 1){
        ...
        df_fit <- summary(fit_time_ctl[[j]])$df[1]
        sigma_imp_ctl <- (df_fit-2)*sigma_ini/rchisq(1, df = df_fit)
        ...
        df_fit <- summary(fit_time_trt[[j]])$df[1]
        sigma_imp_trt <- (df_fit-2)*sigma_ini/rchisq(1, df = df_fit)
    ...
    for(j in 1:p){
      beta_imp_ctl[[j]] <- as.vector(rmvnorm(1, mean = beta_time_ctl[[j]], sigma = beta_cov_ctl[[j]]))
      beta_imp_trt[[j]] <- as.vector(rmvnorm(1, mean = beta_time_trt[[j]], sigma = beta_cov_trt[[j]]))
      # sigma_ini <- sigma_time_ctl[j]
      # df_fit <- summary(fit_time_ctl[[j]])$df[1]
      # sigma_imp_ctl[j] <- (df_fit-2)*sigma_ini/rchisq(1, df = df_fit)
    ...
    for(j in 1:p){
      if(j < p){
        for(ind in (j+1):p){
          sum_term <- 0
          for(i in j:(ind-1)){
            sum_term <- beta_time_ctl[[ind]][i+k+1]*sigma_coef[j,i] + sum_term
          }
          sigma_coef[j,ind] <- sum_term
        }
      }
    }
    component_mat <- apply(sigma_coef, 2, function(x) x^2*sigma_time_ctl)
    sigma_total <- apply(component_mat, 2, sum)
    
    sigma_ctl <- diag(sigma_total)
    for(j1 in 1:(p-1)){
      for(j2 in (j1+1):p){
        for(ind in 1:(j2-1)){
    ...
    mean_ctl <- rep(0, n1)
    mean_trt <- rep(0, n2)
    sigma_mi_ctl <- rwish(p+1, sigma_ctl)/(p+1)
    for(j in 2:p){
      sigma_mo <- sigma_mi_ctl[j:p, 1:(j-1)]%*%solve(sigma_mi_ctl[1:(j-1), 1:(j-1)])
      sigma_pattern[j-1] <- (sigma_mi_ctl[j:p, j:p] - sigma_mo%*%sigma_mi_ctl[1:(j-1),j:p])[p-j+1, p-j+1]
...
rubin_est <- function(db_comb, db_long, chg_imp, M, fit_model){
  ...
  rr2_fn <- function(y){
    ...
    # fit_rr <- lm(wt ~ x1 + x2 + factor(trt), data = db_reg)
    # using bisquare weight
    # fit_ctl <- rlm(wt ~ x1 + x2, data = db_reg_ctl,
    #                psi = psi.bisquare)
    beta_rr <- coef(fit_rr)
...
nonpara_mi_fn <- function(db_comb, db_long, M, B){
  db_comb_ctl <- db_comb[which(db_comb$trt == 1),]
  db_comb_trt <- db_comb[which(db_comb$trt == 2),]
  ...
  for(b in 1:B){
    ...
    chg_imp <- rr_seq_imp(data_boot, data_long, M)
...
main<-function(seed, case){
  ...
  if(missing_type == "MCAR"){
    ...
    tmp2 <- sim_one_group(n = N, k = k, P = p, sigma = Sigma, beta = beta_trt, pi = Pi_trt, trt = 2, missing_type = missing_type, distribution = distribution)
  }
  if(missing_type %in% c("MAR", "MNAR")){
    if(case == "Case0"){
  ...
  minp_res <- nonpara_mi_fn(db_comb, db_long, M, B)
...
## Analysis model ----
# fit_model <- lm
fit_model <- MASS::rlm # RR
# fit_model <- Rfit::rfit # rank regression

...
j2r_llh <- function(db_comb, db_long){
  ...
  # point estimate
  mut_ctl <- beta_transform_ctl[10] + sum(beta_transform_ctl[11:12]*adj_mean)
  mut_trt <- beta_transform_trt[10] + sum(beta_transform_trt[11:12]*adj_mean)
  mu1 <- mut_ctl
  ...
  coef4_trt <- rbind(mat_vec[,6], mat_vec[,9], mat_vec[,12])
  cov4_trt <- coef4_trt%*%cov_beta_trt%*%t(coef4_trt)
  ...
  coef_beta_mat_trt <- matrix(beta_transform_trt[11:12]*obs_pi[2] + beta_transform_ctl[11:12]*(1 - obs_pi[2]), nrow = 1)
  coef_beta_mat_diff <- matrix((beta_transform_trt[11:12] - beta_transform_ctl[11:12])*obs_pi[2], nrow = 1)

...
j2r_llh <- function(db_comb, db_long, M){
  ...
  fit_complete_ctl <- fit_model(chg_ctl ~ x1 + x2, data = db_complete_ctl)
  fit_complete_trt <- fit_model(chg_trt ~ x1 + x2, data = db_complete_trt)
  beta_complete_ctl <- coef(fit_complete_ctl)
  beta_complete_trt <- coef(fit_complete_trt)
  cov_complete_trt <- vcov(fit_complete_trt)
  
  # point estimate
  mu_mar_ctl <- sum(coef1_mat*beta_ctl) # MAR
  mu_ctl <- mu_mar_ctl
  mu_complete_trt <- sum(coef2_mat*beta_complete_trt) # Complete
  ...
  coef_beta_mat_trt <- matrix(beta_complete_trt[2:(k+1)]*obs_pi[2] + beta_ctl[2:(k+1)]*(1 - obs_pi[2]), nrow = 1)
  coef_beta_mat_diff <- matrix((beta_complete_trt[2:(k+1)] - beta_ctl[2:(k+1)])*sqrt(obs_pi[2]^2 + obs_pi[2]*(1 - obs_pi[2])/n2), nrow = 1)
